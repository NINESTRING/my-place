...
사진을 고르면 EXIF 좌표에 마커를 찍은 읽기 전용 지도를 폼 안에 보여준다. 마이그레이션 전 `pages/create.tsx`가 하던 동작(`panTo`로 촬영 위치로 이동)을 유지한 것으로, 저장되는 좌표는 여전히 EXIF에서만 오고 지도는 편집 수단이 아니다.

이 항목은 최초 계획에서 누락되었다가 최종 리뷰에서 기능 회귀로 지적되어 복원되었다.
